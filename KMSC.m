...
for m = 1:length(D)
    ...
    [del_m,delm,~] = svd(KTBTB);
    
    [T,eT,~] = svd(KTmTm, 'econ');
    [B,eB,~] = svd(KBB, 'econ');
    
    for nBT= 1:size(delm,1)
        if(trace(delm(1:nBT,1:nBT).^2)/trace(delm.^2)>per) 
            break
        end
    end
    for nT = 1:size(eT,1)
        if(trace(eT(1:nT,1:nT).^2)/trace(eT.^2)>per) 
            break
        end
    end
    for nB = 1:size(eB,1)
        if(trace(eB(1:nB,1:nB).^2)/trace(eB.^2)>per) 
            break
        end
    end
